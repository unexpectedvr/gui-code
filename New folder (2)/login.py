...
class Register:
    def __init__(self,root):
        ...
        paragraph = Label(left, text="New here create?" , font=("time new roman" ,15,"bold"),bg="black",fg="white").place(x=180,y=300)
        paragraph = Label(left, text="An account below!" , font=("time new roman" ,15,"bold"),bg="black",fg="white").place(x=180,y=340)

        frame1 = Frame(self.root, bg ="white")
        frame1.place(x=480 , y=100 , width=700 , height=500)

        title = Label(frame1, text="Sign-In" , font=("time new roman" ,22,"bold"), bg="black" ,fg="white").place(x=250,y=30)
        ...
        self.reg = Button(frame1, bd="5", text="Sign In", font=("time new roman" ,20),command=self.register_d, cursor="hand2", bg="black", fg="white").place(x=280, y=430)

        btn_login = Button(self.root, bd="7", text="Register", font=("time new roman", 20), bg="white",fg="black").place(x=180, y=480)
        ...
        if self.txt_email.get() == "" or self.txt_password.get() == "" or self.txt_conpass.get() == "":
           ...
           messagebox.showerror("warning","Passwords Do Not Match",parent=self.root)
            ...
            try:
                ...
                cur = con.cursor()
                cur.execute("select * from login_table1 where email=%s and password=%s",
                (
                ...
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("error", "Invalid credintials",parent=self.root)
                else:
